...
station_data = pd.read_excel(r"station_data_visualization/data/東京近郊駅データ.xlsx",sheet_name="施設ポートフォリオ")
...
for i in range(len(station_data_2)):
    if math.isnan(station_data_2["病院・一般診療所数"][i]) == False:

        data = (station_data_2['lat'][i],station_data_2['lon'][i],station_data_2["病院・一般診療所数"][i]/10)
